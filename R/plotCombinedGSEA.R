...
#'
#' @param GSEA_results A named list of data frames, where each data frame
#'   contains GSEA results for a contrast. Each data frame should have the
#'   columns: `NES` (Normalized Enrichment Score), `padj` (adjusted p-value),
#'   and `pathway` (pathway name). Output from \code{runGSEA}.
#' @param sig_threshold Numeric, default = 0.05. Adjusted p-value threshold for
...
#' @param widthlegend Numeric, default = 16. Controls the width of pathway
...
plotCombinedGSEA <- function(GSEA_results, sig_threshold = 0.05, PointSize = 4,
  ...
  combined_data$significance <- ifelse(combined_data$padj < sig_threshold,
  ...
  plot <- ggplot2::ggplot(combined_data, ggplot2::aes(x = NES, y = logpadj,
    ...
    # Different shapes for significant and non-significant points
    ggplot2::scale_shape_manual(values = c(15:(15 -1 + length((unique(combined_data$contrast)))))) +
