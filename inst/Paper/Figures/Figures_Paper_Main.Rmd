...
```{r fig.width=12, fig.height=4}
(plt_expectations <- ggarrange(plt_expectation_scores,plt_expectation_gsea, labels=c("A","B"), widths=c(0.6,0.4)))
```
...
plt_senequest <- ggarrange(plt_tstats,plot_logmedian_SeneQuest, ncol=1, heights=c(0.45,0.6), labels=c("C","D"))
...
```{r}
ggplot2::ggsave("Figs/Figure5_ComparisonSigs.png",
...
<!-- ```{r} -->
<!-- options(error=recover) -->
<!-- metadata_GTEx_Skin_tissuenames <- metadata_GTEx_Skin -->
<!-- metadata_GTEx_Skin_tissuenames$SunExposure <- gsub("Skin - Sun Exposed (Lower leg)" ,"Sun Exposed",metadata_GTEx_Skin_tissuenames$SMTSD, fixed = TRUE) -->
<!-- metadata_GTEx_Skin_tissuenames$SunExposure <- gsub("Skin - Not Sun Exposed (Suprapubic)","Not Sun Exposed",metadata_GTEx_Skin_tissuenames$SunExposure, fixed = TRUE) -->
<!-- metadata_GTEx_Skin_tissuenames$COHORT <- gsub("Organ Donor (OPO)","Organ Donor",metadata_GTEx_Skin_tissuenames$COHORT, fixed = TRUE) -->
<!-- metadata_GTEx_Skin_tissuenames$SEX <- gsub(1,"Male",metadata_GTEx_Skin_tissuenames$SEX, fixed = TRUE) -->
<!-- metadata_GTEx_Skin_tissuenames$SEX <- gsub(2,"Female",metadata_GTEx_Skin_tissuenames$SEX, fixed = TRUE)  -->

<!-- GSEA_VariableAssociation(data=GTEx_Skin,  -->
<!--                          metadata=metadata_GTEx_Skin_tissuenames,  -->
<!--                          cols=c("AGE","SunExposure","SEX"), # SMRIN was a variable for batch correction -->
<!--                          mode="simple",  -->
<!--                          gene_set=list(HernandezSegura=signatures_bidirectional$HernandezSegura),  -->
<!--                          saturation_value=0.001, nonsignif_color = "white", signif_color = "red", -->
...
<!--                          cols=c("AGE","SunExposure","SEX"), # SMRIN was a variable for batch correction -->
<!--                          mode="simple",  -->
<!--                          gene_set=list(SAUL_SEN_MAYO=signatures_bidirectional$SAUL_SEN_MAYO),  -->
<!--                          saturation_value=0.001, nonsignif_color = "white", signif_color = "red", -->
<!--                          sig_threshold = 0.05, widthlabels=30, labsize=10, titlesize=14, pointSize = 5)  -->


<!-- ``` -->
<!-- This work suggests cohen's d < 1 as not meaningful. -->
<!-- https://pmc.ncbi.nlm.nih.gov/articles/PMC8916770/?utm_source=chatgpt.com -->
...
<!-- Score_VariableAssociation(data=GTEx_Skin,  -->
<!--                           metadata=metadata_GTEx_Skin_tissuenames,  -->
<!--                           cols=c("AGE","SunExposure","SEX"), # SMRIN was a variable for batch correction -->
<!--                           method="logmedian",  -->
<!--                           gene_set = list(HernandezSegura=signatures_bidirectional$HernandezSegura),  -->
...
<!--                           widthlabels=30, labsize=10, title=NULL, titlesize=12, pointSize=5, discrete_colors=NULL, -->
<!--                           continuous_color = "#7AACAF", color_palette = "Set2") -->
...
results_df_score_corr <- results_df_score %>%
  group_by(signature, method) %>%
  mutate(P_adj = p.adjust(P_Value, method = "BH")) %>%
  ungroup()

results_df_gsea_corr <- results_df_gsea %>%
  group_by(signature) %>%
  mutate(padj = p.adjust(pval, method = "BH")) %>%
  ungroup()

# Alphabetically order tissue factor
tissue_levels <- sort(unique(results_df_score_corr$tissue), decreasing = T)
results_df_score_corr$tissue <- factor(results_df_score_corr$tissue, levels = tissue_levels)
results_df_gsea_corr$tissue  <- factor(results_df_gsea_corr$tissue, levels = tissue_levels)
...
results_df_score_corr$method <- factor(results_df_score_corr$method, levels = c("logmedian", "ranking", "ssGSEA"))
results_df_gsea_corr$method <- "GSEA"  # single method name

# Add asterisk for significance
results_df_score_corr <- results_df_score_corr %>%
  mutate(signif = ifelse(P_adj < 0.05, "*", ""))

results_df_gsea_corr <- results_df_gsea_corr %>%
  mutate(signif = ifelse(padj < 0.05, "*", ""))

# Loop through each signature
sigs <- unique(results_df_score_corr$signature)

list_plts_sigs <- list()
for (sig in sigs) {
  # Subset for this signature
  score_data <- results_df_score_corr %>% filter(signature == sig)
  gsea_data  <- results_df_gsea_corr %>% filter(signature == sig)
...
for (tissue in tissues_signif_HS){
  ...
  for (method in methods){ 
    ...
    pltlist_aux[[method]] <- ggplot(scores_df$HernandezSegura, aes(x=AGE, y=score)) +
      ...
      ggplot2::geom_smooth(method = "lm", col = "black", se = FALSE, size=1.3)+ ggpubr::stat_cor(method="spearman", size=3.5, p.digits = 1 ) +
...
```{r}
ggplot2::ggsave("Figs/Figure7_GTEx.png",
                fig7,
                width = 12, height=13, bg = 'white')
```
