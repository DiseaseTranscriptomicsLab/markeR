...
signatures <- readRDS("data/SenescenceSignatures.rds") # Divided by direction
...
```
```{r}
ggplot2::ggsave("Figures/Figs/Figure1_GeneSets_Datasets.png",
                plt_fig1,
...
```{r}
ggplot2::ggsave("Figures/Figs/Figure2_Scores.png",
                plt_fig2,
...
```{r}
ggplot2::ggsave("Figures/Figs/Figure3_Enrichment.png",
                fig3,
...
```{r}
ggplot2::ggsave("Figures/Figs/Figure5_SeneQuest.png",
                plt_fig5_senequest,
...
(results_nopvalcorrection <- readRDS("data/Tradeoffs_v2.rds"))
```
...
```{r}
results_long_pval_correction <- results_long %>%
  group_by(signature, method, gene_pct) %>%
  mutate(p_adj_gene = p.adjust(if_else(method == "Score", p_score, p_enrich), method = "BH")) %>%
  ungroup() %>%
  group_by(signature, method, sample_pct) %>%
  mutate(p_adj_sample = p.adjust(if_else(method == "Score", p_score, p_enrich), method = "BH")) %>%
  ungroup()
```

```{r fig.width=12, fig.height=4}

plt_samplepct_coorect <- ggplot(results_long_pval_correction %>% filter(gene_pct == 100),
       aes(x = sample_pct, y = logp, color = signature)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  facet_wrap(~method, scales = "free") +
...
plt_genepct_coorect <- ggplot(results_long_pval_correction %>% filter(sample_pct == 100),
       aes(x = gene_pct, y = logp, color = signature)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  facet_wrap(~method, scales = "free") +
 ...
 ggarrange(plt_samplepct_coorect,plt_genepct_coorect, common.legend = T, nrow=1, legend="top", labels="AUTO") 
```
...
(fig5_tradeoffs <- ggarrange(plt_tradeoffs,plt_impact_score_cellage, plt_impact_enrichment, ncol=1, heights=c(0.25,0.3,0.45), labels=c("","C","D")))
```
```{r}
ggplot2::ggsave("Figures/Figs/Figure5_tradeoffs.png",
                fig5_tradeoffs,
