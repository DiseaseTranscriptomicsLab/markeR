...
cor_long <- df_combined %>%
  group_by(GeneSet) %>%
  summarise(
    `logmedian vs ssGSEA` = cor(logmedian, ssGSEA, method = "spearman"),
    `logmedian vs ranking` = cor(logmedian, ranking, method = "spearman"),
    `ssGSEA vs ranking` = cor(ssGSEA, ranking, method = "spearman")
  ) %>%
  pivot_longer(-GeneSet, names_to = "Comparison", values_to = "SpearmanRho")


cor_long$GeneSet <-  vapply(cor_long$GeneSet, FUN = function(x) wrap_title(x,20),"character")
...
(plt_violin_methods <- ggplot(cor_long, aes(x = Comparison, y = SpearmanRho)) +
  ...
  theme_minimal() +
  labs(x = "Method Comparison", y = "Spearman ρ" ) +
  theme(
    legend.position = c(0.5, 0.3),   # inside top-right
    legend.title = element_blank(),
    legend.background = element_rect(color = alpha('black', 0.5))  
  ) +
  scale_color_manual(values = geneSet_colors, guide = guide_legend(nrow = 3)) +
  ylim(c(0,1)) )
...
(supfig3_new <- ggarrange(plt_scores_2, plt_scorecorrelations, ncol=1, heights=c(0.8,0.2), labels = "AUTO",font.label = list(size = 20)))
```
```{r}
