...
PlotScores_Categorical_adapted_notitle <- function(data, metadata, gene_sets,
                                           method = c("ssGSEA", "logmedian", "ranking"),
                                           ColorVariable = NULL, GroupingVariable = NULL,
                                           ColorValues = NULL, ConnectGroups = FALSE, ncol = NULL, nrow = NULL, title = NULL,
                                           widthTitle = 10, titlesize = 12, limits = NULL, legend_nrow = NULL, pointSize = 2,
  ...
  if (is.null(GroupingVariable) | is.null(metadata)) {
    ...
    for (signature in names(ResultsList)) {
      ...
      # Customize the plot appearance.
      p <- p + ggplot2::theme_classic() +
        ggplot2::labs(title = NULL, color = "", x = "", y = "") +
        ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1, size = labsize - .5),
                       axis.text.y = ggplot2::element_text(  size = labsize - .5),
                       plot.title = ggplot2::element_text(hjust = 0.5, size = titlesize-1),
                       plot.subtitle = ggplot2::element_text(hjust = 0.5, size = titlesize - 1.5, face = "italic"),
  ...
  for (signature in names(ResultsList)) {
    ...
    p <- p + ggplot2::theme_bw() +
      ggplot2::theme(axis.text.x = ggplot2::element_text(angle = 45, hjust = 1, size = labsize),
                     ...
                     plot.title = NULL,
                     plot.subtitle = NULL) +
      ggplot2::labs(title = NULL, subtitle = NULL, color = "", x = "", y = "")
...
PlotScores_adapted_notitle <- function(data, metadata, gene_sets,
                               method = c("ssGSEA", "logmedian", "ranking", "all"),
                               ColorVariable = NULL, Variable = NULL,
                               ColorValues = NULL, ConnectGroups = FALSE, ncol = NULL, nrow = NULL, title = NULL,
                               widthTitle = 20, titlesize = 12, limits = NULL, legend_nrow = NULL, pointSize = 4,
  ...
  if (method == "all") { # returns heatmap
    ...
    if (type!="Numeric"){
      ...
      return(
        ...
        PlotScores_Categorical_adapted_notitle(data=data, metadata=metadata, gene_sets=gene_sets,
                                       method = method,
                                       ColorVariable = ColorVariable, GroupingVariable = Variable,
                                       ColorValues = ColorValues, ConnectGroups = ConnectGroups, ncol = ncol, nrow = nrow, title = title,
                                       widthTitle = widthTitle, titlesize = titlesize, limits = limits, legend_nrow = legend_nrow, pointSize = pointSize,
...
Heatmap_Cohen_adapted <- function(cohenlist, nrow = NULL, ncol = NULL, limits = NULL,
  ...
  for (signature in names(cohenlist)) {
    ...
    long_data$Var2 <- factor(long_data$Var2, levels = c("logmedian", "ranking", "ssGSEA"))
...
```{r}
PlotScores_Categorical_adapted_noX_noleg_notitle <- function(data, metadata, gene_sets,
                                           method = c("ssGSEA", "logmedian", "ranking"),
                                           ColorVariable = NULL, GroupingVariable = NULL,
                                           ColorValues = NULL, ConnectGroups = FALSE, ncol = NULL, nrow = NULL, title = NULL,
                                           widthTitle = 10, titlesize = 12, limits = NULL, legend_nrow = NULL, pointSize = 2,
  ...
  for (signature in names(ResultsList)) {
    ...
    p <- p + ggplot2::theme_bw() +
      ggplot2::theme(axis.text.x = ggplot2::element_blank(),
                     ...
                     plot.subtitle = NULL,
                     legend.position = "none") +
      ggplot2::labs(title = NULL, subtitle = NULL,x = "", y = "")  
...
PlotScores_adapted_noX_noleg_notitle <- function(data, metadata, gene_sets,
                               method = c("ssGSEA", "logmedian", "ranking", "all"),
                               ColorVariable = NULL, Variable = NULL,
                               ColorValues = NULL, ConnectGroups = FALSE, ncol = NULL, nrow = NULL, title = NULL,
                               widthTitle = 20, titlesize = 12, limits = NULL, legend_nrow = NULL, pointSize = 4,
  ...
  if (method == "all") { # returns heatmap
    ...
    if (type!="Numeric"){
      ...
      return(
        ...
        PlotScores_Categorical_adapted_noX_noleg_notitle(data=data, metadata=metadata, gene_sets=gene_sets,
                                       method = method,
                                       ColorVariable = ColorVariable, GroupingVariable = Variable,
                                       ColorValues = ColorValues, ConnectGroups = ConnectGroups, ncol = ncol, nrow = nrow, title = title,
                                       widthTitle = widthTitle, titlesize = titlesize, limits = limits, legend_nrow = legend_nrow, pointSize = pointSize,
...
df_pval_melt <- reshape2::melt(df_pval)
colnames(df_pval_melt) <- c("Pathway", "Signature", "pval")

df_OR_melt <- reshape2::melt(df_OR)
...
(plt_sup2_alternative <- ggarrange(final_plot_localisation2,plt_logodds, nrow=1, labels = "AUTO", font.label = list(size = 20), widths = c(0.6,0.5)))
```


```{r}
ggplot2::ggsave("Figs/SupFigure2_GeneSets_Function_Localisation.png",
                plt_sup2_alternative,
                width = 15, height=14, bg = 'white')

ggplot2::ggsave("Figs/SupFigure2_GeneSets_Function_Localisation.pdf",
                plt_sup2_alternative,
                width = 15, height=14, bg = 'white')
```
...
flat_df_scores$Method <- ifelse(flat_df_scores$Method == "logmedian", "log2-median", flat_df_scores$Method)

```
```{r fig.width=16, fig.height=12}

# Custom labeller returning expressions as bold
label_method <- function(value) {
  sapply(value, function(v) {
    if (v == "log2-median") {
      "bold(log[2]*'-median')"   # bold + subscript 2
    } else {
      paste0("bold('", v, "')")  # other labels bold
    }
  })
}
...
for (sig in signatures) {
  ...
  p <- ggplot(df_sig, aes(x = Condition, y = score)) +
    ...
    facet_wrap(~ Method, scales = "free_y",
             labeller = labeller(Method = label_method, .default = label_parsed)) +
...
p1 <- ggplot(df_combined, aes(x = logmedian, y = ssGSEA)) +
 ...
 guides(color = guide_legend(nrow = 3, override.aes = list(size = 4))) +
  ...
  labs(title = expression(log[2]*"-median vs ssGSEA"), x = expression(log[2]*"-median score"), y = "ssGSEA score") +
  theme(
    legend.position = "bottom",
...
p2 <- ggplot(df_combined, aes(x = logmedian, y = ranking)) +
  ...
  scale_color_manual(values = darker_geneSet_colors) +
  theme_classic(base_size=16) +
  labs(title = expression(log[2]*"-median vs ranking"), x = expression(log[2]*"-median score"), y = "ranking score") +
  theme(
    legend.position = "bottom",
    legend.background = element_rect(fill = "white"),
    legend.title = element_blank())
...
geneSet_colors2 <- geneSet_colors
names(geneSet_colors2) <- unique(cor_long$GeneSet)[order(unique(cor_long$GeneSet))]

# cor_long$GeneSet <- ifelse(cor_long$GeneSet=="GOBP_NEGATIVE_REGULATION_\nOF_CELLULAR_SENESCENCE", "\nGOBP_NEGATIVE_REGULATION_\nOF_CELLULAR_SENESCENCE\n",cor_long$GeneSet)
# 
# names(geneSet_colors2) <- ifelse(names(geneSet_colors2)=="GOBP_NEGATIVE_REGULATION_\nOF_CELLULAR_SENESCENCE", "\nGOBP_NEGATIVE_REGULATION_\nOF_CELLULAR_SENESCENCE\n",names(geneSet_colors2))
# 
# cor_long$GeneSet <- factor(cor_long$GeneSet, levels=c("CellAge","CSgene","GOBP_CELLULAR_SENESCENCE","\nGOBP_NEGATIVE_REGULATION_\nOF_CELLULAR_SENESCENCE\n", "GOBP_POSITIVE_REGULATION_\nOF_CELLULAR_SENESCENCE" ,"HernandezSegura","REACTOME_CELLULAR_SENESCENCE","SAUL_SEN_MAYO","SeneQuest" ))


# -----------------------------
# Map Comparison names to expressions for parsing
# -----------------------------
comparison_labels <- c(
  "log2-median \nvs ssGSEA"  = "atop(log[2]*'-median','vs ssGSEA')",
  "log2-median \nvs ranking" = "atop(log[2]*'-median','vs ranking')",
  "ssGSEA \nvs ranking"      = "atop('ssGSEA','vs ranking')"
)

cor_long$Comparison <- factor(cor_long$Comparison, levels = names(comparison_labels))
...
plt_violin_methods <- ggplot(cor_long, aes(x = Comparison, y = SpearmanRho)) +
  ...
  scale_color_manual(values = geneSet_colors2) +
  scale_x_discrete(labels = function(x) parse(text = comparison_labels[x])) +
  ylim(0,1) +
  theme(plot.margin = margin(5,5,5,5))

# # -----------------------------
# # Dummy legend on the right
# # -----------------------------
# gs_names <- unique(cor_long$GeneSet)[order(unique(cor_long$GeneSet))]
# legend_df <- data.frame(
#   x = 1,
#   y = seq(length(gs_names), 1),
#   GeneSet = factor(gs_names, levels = gs_names)
# )
# 
# label_offset <- 0.3
# x_right <- 1 + label_offset + max(nchar(as.vector(gs_names))) * 0.08
# if (x_right < 2.5) x_right <- 2.5
# 
# dummy_legend_plot <- ggplot(legend_df, aes(x=x, y=y)) +
#   geom_point(aes(color = GeneSet), size = 4) +
#   geom_text(aes(label = GeneSet), hjust=0, nudge_x=label_offset, size=4) +
#   scale_color_manual(values = geneSet_colors, guide="none") +
#   theme_void() +
#   coord_cartesian(xlim = c(0.8, x_right), clip = "off") +
#   theme(plot.margin = margin(5, 20, 5, 5))



# ------------------------------------------
# Create ordered gene-set names
# ------------------------------------------
gs_names <- unique(cor_long$GeneSet)[order(unique(cor_long$GeneSet))]

# Base y positions (top to bottom)
y_base <- seq(length(gs_names), 1)

# ------------------------------------------
# Identify the two GOBP entries
# ------------------------------------------
idx_neg <- which(gs_names == "GOBP_NEGATIVE_REGULATION_\nOF_CELLULAR_SENESCENCE")
idx_pos <- which(gs_names == "GOBP_POSITIVE_REGULATION_\nOF_CELLULAR_SENESCENCE")

# ------------------------------------------
# Increase spacing between negative → positive
# and keep spacing uniform below
# ------------------------------------------
gap <- 0.35

# shift positive and all *below* it
y_base[idx_pos:length(gs_names)] <- y_base[idx_pos:length(gs_names)] - gap
...
# -----------------------------
# Combine main plot and dummy legend
# -----------------------------
...
(plt_scorecorrelations <- ggarrange(plt_corr_methods, final_combined, nrow=1, widths = c(0.65,0.4)))
```
...
plt_logmedian_stressors_1 <-  PlotScores_adapted_noX_noleg_notitle(data = corrcounts, 
           metadata = metadata, 
           method = "logmedian", 
...
plt_logmedian_stressors_2 <-  PlotScores_adapted_notitle(data = corrcounts, 
           metadata = metadata, 
           method = "logmedian", 
...
(plt_logmedian_stressors <- ggarrange(plt_logmedian_stressors_1,plt_logmedian_stressors_2, ncol=1, heights=c(0.8,0.2) ))
(plt_logmedian_stressors <- ggpubr::annotate_figure(plt_logmedian_stressors, left = grid::textGrob(expression("Gene Set's Score (" * log[2] * "-median)"), rot = 90, gp = gpar(cex = 1.2))))

plt_ssGSEA_stressors_1 <-  PlotScores_adapted_noX_noleg_notitle(data = corrcounts, 
           metadata = metadata, 
           method = "ssGSEA", 
           gene_sets = signatures_bidirectional[1:8],  
           ColorVariable = "CellType", 
...
plt_ssGSEA_stressors_2 <-  PlotScores_adapted_notitle(data = corrcounts, 
           metadata = metadata, 
           method = "ssGSEA", 
           gene_sets = signatures_bidirectional[9],  
           ColorVariable = "CellType", 
...
(plt_ssGSEA_stressors <- ggarrange(plt_ssGSEA_stressors_1,plt_ssGSEA_stressors_2, ncol=1, heights=c(0.8,0.2)))
...
plt_ranking_stressors_1 <-  PlotScores_adapted_noX_noleg_notitle(data = corrcounts, 
           metadata = metadata, 
           method = "ranking", 
           gene_sets = signatures_bidirectional[1:8],  
           ColorVariable = "CellType", 
...
plt_ranking_stressors_2 <-  PlotScores_adapted_notitle(data = corrcounts, 
           metadata = metadata, 
           method = "ranking", 
           gene_sets = signatures_bidirectional[9],  
           ColorVariable = "CellType", 
...
(plt_ranking_stressors <- ggarrange(plt_ranking_stressors_1,plt_ranking_stressors_2, ncol=1, heights=c(0.8,0.2), common.legend = T, legend = "none"))
...
#                          common.legend=T, nrow=1, widths=c(0.3,0.04,0.3,0.04,0.3), labels = c("A","","B","","C"),font.label = list(size = 16))
...
# log2 in subscript
title_logmedian <- text_grob(
    bquote(bold(log[2] * "-median")),
    size = 14
)

title_ssgsea <- text_grob(
    "         ssGSEA",
    size = 14,
    face="bold"
)

title_ranking <- text_grob(
    "         ranking",
    size = 14,
    face="bold"
...
# ---------------------------------------------------------------
# Wrap each plot individually with annotate_figure()
# Each receives its own top title.
# ---------------------------------------------------------------

plt_logmedian_stressors_titled <- annotate_figure(
    plt_logmedian_stressors,
    top = title_logmedian
)

plt_ssGSEA_stressors_titled <- annotate_figure(
    plt_ssGSEA_stressors,
    top = title_ssgsea
)

plt_ranking_stressors_titled <- annotate_figure(
    plt_ranking_stressors,
    top = title_ranking
)

# ---------------------------------------------------------------
# Assemble final figure with NULL spacers as before
# ---------------------------------------------------------------

supfigure_3 <- ggarrange(
    plt_logmedian_stressors_titled,
    NULL,
    plt_ssGSEA_stressors_titled,
    NULL,
    plt_ranking_stressors_titled,
    common.legend = TRUE,
    nrow = 1,
    widths = c(0.3, 0.04, 0.3, 0.04, 0.3),
    labels = c("A", "", "B", "", "C"),
...
# Your gene set labels
geneset_labels <- names(signatures_bidirectional)[order(names(signatures_bidirectional))]
geneset_labels <- unlist(lapply(geneset_labels, function(x) wrap_title(x, 30)))
n_genes <- length(geneset_labels)

dummydf <- data.frame(
  y = c(9.1,8.2,7.4,6.5,5.7,4.8,4,3.1,2.2),
  x = 0.2,
  labelgs = geneset_labels
)
# Create a dummy plot with the gene set names
labels_plot <- ggplot(dummydf, aes(x = x, y = y, label = labelgs)) +
  geom_text(hjust = 0, size = 4, fontface = "bold") +
  scale_y_continuous(limits = c(0.5, n_genes + 0.5), breaks = NULL, expand = c(0,0)) +
  scale_x_continuous(limits = c(0, 2), expand = c(0,0)) +  # add some space for the text
  theme_void()  +
  theme(plot.margin = margin(0, 5, 0, 0)) 

# Combine your existing grid with the labels column
final_grid <- ggarrange(
  supfigure_3,
  labels_plot,
  ncol = 2,
  widths = c(0.7, 0.2)  # adjust widths as needed
)
...
plt_fdrsim_allsignatures_v3 <-readRDS("../data_aux/FPRsim_100_data_and_plot_Dec1_2025_allsigs.rds")
...
# results_df_gsea <- readRDS("../data_aux/results_df_gsea_GTEx_FPR.rds")
  ...
  results_df_score <- readRDS("data_aux/results_df_score_GTEx_FPR_agecentered_extendedGTExdata.rds")
results_df_gsea <- readRDS("data_aux/results_df_gsea_GTEx_FPR_agecentered_extendedGTExdata.rds")
...
tissue_levels <- sort(unique(results_df_score$tissue), decreasing = F)
...
for (tissue in tissues_signif_SenMayo){
  ...
  for (method in methods){ 
    ...
    pltlist_aux[[method]] <- ggplot(scores_df$SAUL_SEN_MAYO, aes(x=AGE, y=score)) +
      ...
      theme_bw() +
      ggtitle(method_title) +  # <--- formatted title
      theme(plot.title = element_text(hjust = 0.5),
            base_size = 16,
...
(plotNESlollipop_senmayo <- plotNESlollipop_fdr(GSEA_results=GSEAresults_senmayo, sig_threshold = 0.05,saturation_value=0.00001, nonsignif_color = "white", signif_color = "#934873",
...
results_df_gsea_SenMayoCustom$signature <- ifelse(results_df_gsea_SenMayoCustom$signature=="SAUL_SEN_MAYO_WO_INFLAMMATION", "Non-inflammatory SAUL_SEN_MAYO","Inflammatory SAUL_SEN_MAYO")

results_df_score_SenMayoCustom$signature <- ifelse(results_df_score_SenMayoCustom$signature=="SAUL_SEN_MAYO_WO_INFLAMMATION", "Non-inflammatory SAUL_SEN_MAYO","Inflammatory SAUL_SEN_MAYO")
