...
GTEx_alltissues_filt <- readRDS("../data_aux/GTExV8_voyagercorrected.rds") # https://github.com/DiseaseTranscriptomicsLab/voyAGEr/tree/main/Corrected_Counts
...
```{r}
# df_Scores_alltissues <- data.frame(NULL)
# for (tissue in unique(metadata_GTEx_alltissues$SMTSD)){
#   
...
#   
# df_Scores_tissue <- CalculateScores(data = subset_data,
#                                         metadata = subset_metadata, # sample id has to be first column
#                                         method = "ranking",
#                                         gene_sets =  list(HernandezSegura = signatures_bidirectional$HernandezSegura))
# 
# df_Scores_alltissues <- rbind(df_Scores_alltissues, df_Scores_tissue$HernandezSegura)
# 
# }
# df_Scores_alltissues


df_Scores_alltissues <- CalculateScores(data = GTEx_alltissues_filt,
                                        metadata = metadata_GTEx_alltissues, # sample id has to be first column
                                        method = "logmedian",
                                        gene_sets =  list(HernandezSegura = signatures_bidirectional$HernandezSegura))$HernandezSegura

```

 
```{r fig.width=12, fig.height=5}
(plt_gtex_score_alltissues <- ggplot(df_Scores_alltissues, aes(x = SMTSD, y = score)) +
   geom_jitter(color="#DDDDDD") +
   # add dashed horizontal line for median across all tissues
   geom_hline(yintercept = median(df_Scores_alltissues$score), linetype = "dashed", color = "#888888") +
...
tissues_signif_HS <- c("Artery - Aorta","Brain - Nucleus accumbens (basal ganglia)", "Thyroid", "Uterus")
tissues_signif_SenMayo <- c("Artery - Aorta",  "Artery - Tibial", "Colon - Sigmoid", "Uterus", "Whole Blood")
...
for (tissue in tissues_signif_HS){
  ...
  subset_metadata <- metadata_GTEx_alltissues[metadata_GTEx_alltissues$SMTSD == tissue,]
  subset_data <- GTEx_alltissues[[tissue]]
  subset_data <- subset_data[,subset_metadata$SAMPID]
