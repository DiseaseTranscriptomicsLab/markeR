...
# Arrange plots
plt_tstats <- ggarrange(plt_scatter_SeneQuest, plt_scatter_HernandezSegura, plt_scatter_REACTOME, nrow=1, common.legend = T)
plt_tstats
```
...
plt_fdrsim_subset_v3 <- readRDS("../data_aux/FPRsim_100_data_and_plot_Sept30_2025_allsigs.rds")
...
plt_senequest <- ggarrange(plt_scores,blank,plt_tstats, ncol=1, heights=c(0.6,0.02,0.8), labels=c("","E"))
...
(plt_fig5_alternative <- ggarrange(
  ...
  labels = c("", "", "", "F"),
  ncol = 1,
  heights = c(0.26, 0.5, 0.02, 0.2)   # adjust 0.04 to control spacing
))
```
...
for (tissue in tissues_signif_HS){
  ...
  for (method in methods){ 
    ...
    pltlist_aux[[method]] <- ggplot(scores_df$HernandezSegura, aes(x=AGE, y=score)) +
      ...
      ggplot2::geom_smooth(method = "lm", col = "black", se = FALSE, size=1.3)+ ggpubr::stat_cor(aes(label = ..r.label..), method = "spearman",
                                                                                                     cor.coef.name="rho",  
                                                                                                 label.x = 20, size=4) +  
